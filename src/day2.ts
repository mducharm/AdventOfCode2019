...
const setInputs = (a: number, b: number, intCodes: number[]) => {
    return [intCodes[0], a, b, ...intCodes.slice(3)]
}

console.log(
    processCodes(setInputs(12, 2, intCodes))
);
...
const getInputsByOutput = (arr: number[], desiredOutput: number) => {
    arr.forEach(n => {
        arr.forEach(m => {
            if (processCodes(setInputs(n, m, intCodes)) === desiredOutput) {
                console.log(processCodes(setInputs(n, m, intCodes)));
                console.log(`${n} ${m}`);
            }
        })
    });
...
getInputsByOutput(arr, 19690720)
