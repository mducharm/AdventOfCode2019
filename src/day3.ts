...
const parseWireString = (wire: string): CoordinatePair[] => {
    let segments = wire.split(',');
    return segments.map(segment => createCoordinate(segment));
}

const createCoordinate = (path: string): CoordinatePair => {
    let [direction, ...numArray] = path.split('');

    let num = Number(numArray.reduce((a, n) => a + n, ''))

    let coord = determineDirection(direction)(num)

    return coord;
}

const determineDirection = (direction: string): Function => {
    if (direction === Direction.D) return down;
    else if (direction === Direction.U) return up;
    else if (direction === Direction.R) return right;
    else if (direction === Direction.L) return left;
    else return () => { };
}

const right = (num: number): CoordinatePair => { return { x: num, y: 0 } }
const up = (num: number): CoordinatePair => { return { x: 0, y: num } }
const down = (num: number): CoordinatePair => { return { x: 0, y: -num } }
const left = (num: number): CoordinatePair => { return { x: -num, y: 0 } }

createCoordinate("R991");